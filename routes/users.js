...
router.put('/:userId/shoppingcart/:productId', async (req,res)=>{
    try{
        ...
        const user =await User.findById(req.params.userId);
        if(!user) return res.status(400).send(`The user with the id "${req.params.productId}" does not exist.`);
...
router.delete('/:userId/shoppingcart/:productId', async (req,res)=>{
    try{
        ...
        if (!product) return res.status(400).send(`The product with id"${req.params.productId}"does not exist in the users shopping cart.`);
